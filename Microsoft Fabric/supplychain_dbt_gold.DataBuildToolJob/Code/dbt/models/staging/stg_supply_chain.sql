...
with source as (
    ...
    {% if is_incremental() %}
    where try_convert(date, order_date, 101) > (select max(order_date) from {{ this }})
    {% endif %}
...
Cleansed as (
    ...
    select
        ...
        case
            when Customer_State not like '%[^0-9]%' then null
        ...
        try_convert(date, shipping_date, 101) as shipping_date,
        trim(Shipping_Mode)                                      as shipping_mode,
 
        try_convert(date, Movement_Date, 101) as movement_date,
